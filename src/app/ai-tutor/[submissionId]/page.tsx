...
export default function AITutorPage({ params }: { params: { submissionId: string } }) {
  ...
  const fetchExamData = useCallback(async (submissionId: string) => {
    if (!user) return;
    try {
  ...
  }, [pageLoading, submission, questions, startTutorSession]);
          ...
          <div className="space-y-6">
            {messages.map((message, index) => (
              ...
              >
                ...
                <div
                  className={cn(
                    'prose prose-sm dark:prose-invert max-w-[75%] rounded-lg p-3 text-sm',
                    message.role === 'user' ? 'bg-primary text-primary-foreground prose-p:text-primary-foreground' : 'bg-muted'
                  )}
                >
        ...
        <div className="p-4 border-t">
          <form onSubmit={handleSubmit} className="flex items-center gap-2">
            <Input
              ...
              className="flex-1"
              disabled={isLoading || !user || !submission}
            />
