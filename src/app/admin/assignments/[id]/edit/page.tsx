...
type EditAssignmentPageProps = PageProps<{ id: string }>;
...
export default function EditAssignmentPage({ params }: EditAssignmentPageProps) {
  ...
  useEffect(() => {
    async function fetchCoursesAndAssignment() {
        setPageLoading(true);
        try {
            // Fetch courses
    ...
    fetchCoursesAndAssignment();
  }, [assignmentId, form, router, toast]);

  
  const onSubmit = async (data: AssignmentFormData) => {
    setIsSubmitting(true);
    try {
      await updateAssignmentAction({ assignmentId, ...data });
