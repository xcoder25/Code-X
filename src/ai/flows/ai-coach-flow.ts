...
export async function chatWithElara(input: ChatWithElaraInput): Promise<ChatWithElaraOutput> {
    const {output} = await elaraPrompt.generate({
        input,
...
const elaraPrompt = ai.definePrompt({
    name: 'elaraPrompt',
    input: {schema: ChatWithElaraInputSchema},
    output: {schema: ChatWithElaraOutputSchema},
    system: `You are Elara, an expert, friendly, and encouraging AI learning coach for the Code-X platform. Your goal is to provide personalized guidance, clarify concepts, and help users on their coding journey.
